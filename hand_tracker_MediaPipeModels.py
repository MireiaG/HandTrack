...
Original version in the address: https://github.com/wolterlw/hand_tracking
...
class HandTracker():
    ...
    def predict_joints(self, img_norm):
        ...
        if joints.shape[1]%2 == 0: # if dimension 2
    ...
    def __call__(self, img):
        ...
        # projecting box coordinates into the image
        box_orig = (self._target_box @ Minv.T)[:,:2]
        box_orig -= pad[::-1]
        
        # projecting keypoints back into original image coordinate space
        if joints.shape[1]%2 == 0: # if 2D
            kp_orig = (self._pad1(joints) @ Minv.T)[:,:2]
            kp_orig -= pad[::-1]
        else: #if 3D            
            Zjoints = np.array([joints[:,2]])
            XYjoints = joints
            XYjoints[:,2] = 1
            
            kp_orig_XY = (XYjoints @ Minv.T)[:,:2]
            kp_orig_XY -= pad[::-1]
            kp_orig = np.concatenate((kp_orig_XY, Zjoints.T), axis=1)
